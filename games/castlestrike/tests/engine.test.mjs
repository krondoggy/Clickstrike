...
import { FACTIONS, UNITS, RESEARCH, WAVE_INTERVAL } from '../src/data.js';
...
test('mines generate income and equipment upgrades apply to future waves', () => {
  ...
  assert.equal(g.state.income, 9.2);
...
test('support healing and the shrine reward actual battlefield positioning', () => {
  ...
  assert.equal(g.state.income, 8.8);
});
...
test('restore rejects malformed and incompatible data without executing it', () => {
  const g = createGame();
  for (const change of [s => s.version = 1, s => s.gold = -1, s => s.roster[0].unitId = '__proto__', s => s.roster[1].id = s.roster[0].id, s => s.research.tier = 10, s => s.structures[0].hp = Infinity, s => s.spellCooldowns = null, s => s.supply = 999]) {
...
test('all research stops at the advertised maximum and never spends on a rejected upgrade', () => {
...
test('a fortified army can destroy the opposing citadel and earn victory', () => {
  const g = createGame(); rich(g);
  for (const r of RESEARCH) for (let i = 0; i < r.maxLevel; i++) g.research(r.id);
...
test('all faction abilities remain finite and serializable in mixed elite battles', () => {
  ...
  for (const faction of FACTIONS) {
    ...
    for (const r of RESEARCH) for (let i = 0; i < r.maxLevel; i++) assert.ok(g.research(r.id).ok);
    ...
    Object.assign(g.state.enemy, { tier: 3, mineLevel: 4, supplyCap: 72, research: { ...g.state.research } });
